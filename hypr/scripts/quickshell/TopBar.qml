    ...
    delegate: Component {
	    ...
	    Connections {
            ...
            Item {
                ...
                Rectangle {
                    ...
                    Row {
                        ...
                        // STR identity mark — bold azure "S" leads the bar.
                        // Sized in line with the bar's icon glyphs (s(22)) and
                        // weighted Black so it reads as a brand mark, not a label.
                        Item {
                            id: strMark
                            width: barWindow.s(36)
                            height: parent.pillHeight
                            anchors.verticalCenter: parent.verticalCenter

                            Text {
                                anchors.centerIn: strMark
                                text: "S"
                            ...
                            // Tiny azure underline gives the mark a "branded" feel
                            // without violating the no-blur / no-gradient rule.
                            Rectangle {
                                anchors.bottom: strMark.bottom
                                anchors.bottomMargin: barWindow.s(6)
                                anchors.horizontalCenter: strMark.horizontalCenter
                ...
                Rectangle {
                    ...
                    Rectangle {
                        ...
                        radius: barWindow.s(10)
                        // STR brand: active workspace pill is azure (the one saturated accent),
                        // not mauve. mocha.mauve still exists for music/secondary widgets.
                        color: mocha.blue
                        z: 0
		    ...
		    Rectangle {
                    ...
                    // STR — Honda Red power button.
                    // FIRST AND ONLY authorized use of #c0392b in the entire shell;
                    // brand discipline requires this to be exclusive. Click runs the
                    // user's existing powermenu.sh (lock / suspend / shutdown / reboot).
                    Rectangle {
                        id: powerBtn
                        property bool isHovered: powerMouseTop.containsMouse
                        anchors.verticalCenter: parent.verticalCenter
                        width: barWindow.s(34)
                        height: barWindow.s(34)
                        radius: width / 2
                        color: powerBtn.isHovered ? "#d44a3b" : "#c0392b"
                        scale: powerBtn.isHovered ? 1.08 : 1.0
                        Behavior on color { ColorAnimation { duration: 200 } }
                        Behavior on scale { NumberAnimation { duration: 200; easing.type: Easing.OutExpo } }

                        Text {
                            anchors.centerIn: powerBtn
                            text: ""
                            font.family: "JetBrainsMono Nerd Font"
                        ...
                        MouseArea {
                            ...
                            onClicked: Quickshell.execDetached(["bash", "-c", "~/.dotfiles/scripts/powermenu.sh"])
                        }
                    }
                }
            }
